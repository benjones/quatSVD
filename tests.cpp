...
template<typename T>
void eigenPolarCorrectnessTests(int N){
  std::vector<Eigen::Matrix<T,3,3>> matrices;
  matrices.reserve(N);
  for(int i = 0; i < N; ++i){
	matrices.push_back(randomEigenMatrix<T>());
  ...
  for(int i = 0; i < N; ++i){
	auto errors = QuatSVD::computeErrorsPolar(matrices[i]);
	totalErrors.push_back(errors.first);
	maxErrors.push_back(errors.second);
  }
...
int main(){
  ...
  std::cout << "tests with eigen for polar decomps" << std::endl;
  std::cout << "doubles " << std::endl;
  eigenPolarCorrectnessTests<double>(N);
  
  std::cout << "floats " << std::endl;
  eigenPolarCorrectnessTests<float>(N);
